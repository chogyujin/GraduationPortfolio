...
public class SnakeView extends View {
    ...
    private final static int DEFAULT_STROKE_COLOR = 0x8078c257;
    private final static int DEFAULT_FILL_COLOR = 0xff78c257;
    ...
    private float maxRangeValue = 0.0f;
    ...
    public void addValue(float value) {
        if (scaleMode == SCALE_MODE_FIXED) {
            if (value < minValue || value > maxValue) {
                throw new IllegalArgumentException("The value is out of min or max limits." + minValue + " " + maxValue);
            }
        }
    ...
    public void setMaxMinRange(float min, float max){
        minRangeValue = min;
        maxRangeValue = max;
    }
    private void configureAttributes(AttributeSet attrs) {
        ...
        strokeWidthInPx = attributes.getDimensionPixelSize(R.styleable.SnakeView_strokeWidth, strokeWidthInPx); //strokeWidthInPx :5
        chartStyle = attributes.getInteger(R.styleable.SnakeView_chartStyle, chartStyle); //chartStyle : FILL_STROKE
        if (scaleMode == SCALE_MODE_FIXED) { //이 조건문은 안들어옴 , scaleMode : auto
        ...
        if (isInEditMode()) { //이 조건문은 안들어옴 , isInEditMode : false
    ...
    private void initializePaint() {
        ...
        fillPaint.setColor(0x8098463f);
        fillPaint.setStyle(Paint.Style.FILL);

        lowPaint = new Paint();
        lowPaint.setFlags(Paint.ANTI_ALIAS_FLAG);
        lowPaint.setColor(0x8098463f);
        lowPaint.setStyle(Paint.Style.STROKE);

        highPaint = new Paint();
        highPaint.setFlags(Paint.ANTI_ALIAS_FLAG);
        highPaint.setColor(0x8098463f);
    ...
    private void calculateDrawingArea(int width, int height) {
        int left = (strokeWidthInPx * 2) + getPaddingLeft(); // 10+?
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!valuesCache.isEmpty()) {
            Path lowPath = buildPath(false, 1);
            canvas.drawPath(lowPath, lowPaint);
            Path highPath = buildPath(false, 2);
            canvas.drawPath(highPath, highPaint);
            float blockY = (drawingArea.bottom-drawingArea.top)/sizeY;
            canvas.drawRect(drawingArea.left, drawingArea.bottom +blockY - ((minRangeValue) * blockY),
                    drawingArea.right, drawingArea.bottom - blockY - ((maxRangeValue) * blockY), fillPaint);
            if (chartStyle == CHART_STILE_STROKE || chartStyle == CHART_STILE_FILL_STROKE) {
                Path strokePath = buildPath(false, 0);
                canvas.drawPath(strokePath, paint);
            }
        }
    }
    private Path buildPath(boolean forFill, int line) {
        int strokePadding = (strokeWidthInPx / 2);// 5
        Path path = new Path();
        if (forFill) {
        ...
        for (int index = 0; index < currentValuesCache.size(); index++) {
            ...
            if(line==0) {
                ...
                x = drawingArea.left + (scaleInX * index);
                y = drawingArea.bottom - ((pathValue) * blockY);
                if (index == 0) {
                    if (forFill) {
                        path.lineTo(x, y);
                    } else {
                ...
                x = drawingArea.left + (scaleInX * index);
                if (line == 1)
                    y = drawingArea.bottom - ((minRangeValue) * blockY);
                else if (line == 2)
                    y = drawingArea.bottom - ((maxRangeValue) * blockY);
            }
            if (index == 0) {
                path.moveTo(x, y);
            } else {
